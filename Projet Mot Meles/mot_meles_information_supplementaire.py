...
def memoriser_disposition_lettres(grille):
    # Cette fonction mémorise la disposition actuelle de lettres dans la grille.
...
def recharger_grille(grille):
    # Cette fonction recharge la grille avec la disposition actuelle de lettres.
...
def changer_theme():
    # Cette fonction régénère la grille avec de nouveaux mots basés sur un autre thème.
...
# Modifier la fonction afficher_message_victoire pour inclure l'option de changer de thème
def afficher_message_victoire(fenetre):
    """
    Cette fonction affiche un message de victoire et enregistre le score du joueur.
    """
    choix = messagebox.askyesno("Félicitations", "Vous avez trouvé tous les mots! Voulez-vous rejouer avec un autre thème?")
    if choix:
        changer_theme()
    else:
        nom_joueur = simpledialog.askstring("Nom du joueur", "Entrez votre nom :")
        if nom_joueur:
            enregistrer_score(nom_joueur, len(mots_trouves))
...
def creer_fenetre_demarrage():
    ...
    # Ajout d'une variable pour suivre le choix de l'utilisateur pour le timer
    avec_timer = tk.BooleanVar()
    # Par défaut, le jeu ne sera pas joué avec un timer
    avec_timer.set(False) 

    bouton_demarrer_sans_timer = tk.Button(fenetre_demarrage, text="Démarrer le jeu sans timer", command=lambda: [fenetre_demarrage.destroy(), creer_menu()])
    bouton_demarrer_sans_timer.pack(pady=5)

    bouton_demarrer_avec_timer = tk.Button(fenetre_demarrage, text="Démarrer le jeu avec timer", command=lambda: [fenetre_demarrage.destroy(), creer_menu()])
    bouton_demarrer_avec_timer.pack(pady=5)
...
# Mettre à jour le chronomètre pendant le jeu
def update_timer(fenetre, start_time):
    """
    Met à jour le chronomètre.
    """
    elapsed_time = time.time() - start_time
    timer_label.config(text=f"Temps écoulé: {int(elapsed_time)} s")
    fenetre.after(1000, lambda: update_timer(fenetre, start_time))

# Chronomètre compte à rebours
def countdown_timer(fenetre, remaining_time):
    """
    Compte à rebours du chronomètre.
    """
    if remaining_time > 0:
        timer_label.config(text=f"Temps restant: {remaining_time} s")
        fenetre.after(1000, lambda: countdown_timer(fenetre, remaining_time - 1))
    else:
        messagebox.showinfo("Fin du temps", "Temps écoulé!")
        fenetre.destroy()
